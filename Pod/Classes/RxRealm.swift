...
public extension NotificationEmitter where Self: RealmCollection {
    ...
    /**
     Returns an `Observable<Array<Self.Iterator.Element>>` that emits each time the collection data changes. The observable emits an initial value upon subscription.
     ...
     - returns: `Observable<Array<Self.Iterator.Element>>`, e.g. when called on `Results<Model>` it will return `Observable<Array<Model>>`, on a `List<User>` it will return `Observable<Array<User>>`, etc.
     */
...
public extension Realm {
    ...
    /**
     ...
     - returns: `AnyObserver<O>`, which you can use to subscribe an `Observable` to
     */
    public func rx_add<O: Sequence where O.Iterator.Element: Object>(update update: Bool = false) -> AnyObserver<O> {
